...
- Feature frontmatter MUST record a recommended model, reasoning effort, and complexity rationale. Before dispatching or resuming feature work, the coordinator MUST resolve and apply the effective model and effort through the host controls, honoring explicit user overrides. Never silently inherit host defaults or treat Markdown metadata as applied configuration. Follow the mandatory [execution settings gate](references/model-selection.md#apply-at-execution); recommendations do not authorize execution or delegation.
